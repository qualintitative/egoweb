...
class Study extends \yii\db\ActiveRecord
{
    ...
    {
        $multiStudyIds = $this->multiStudyIds();
        $studies = Study::findAll(array('id'=>$multiStudyIds));
        foreach ($studies as $study) {
            $studyNames[$study->id] = $study->name;
        }
        $questions = Question::findAll(array('studyId'=>$multiStudyIds));
    ...
    public static function replicate($study, $questions, $options, $expressions, $alterPrompts = array(), $alterLists = array())
    {
        ...
        foreach ($options as $option) {
            ...
            if (!$newOption->save()) {
                throw new BadRequestHttpException("Option: " . print_r($newOption->errors));
            } //return false;
            else {
        ...
        foreach ($expressions as $expression) {
            ...
            if (!$newExpression->save()) {
                throw new BadRequestHttpException("Expression: " . print_r($newExpression->errors));
            } //return false;
            else {
