   ...
   //             'class' => 'yii\rest\OptionsAction',
    ...
    private function saveAnswersMerge($data, $newData)
    {
            ...
            foreach ($data['alters'] as $alter) {
                ...
                if (!isset($alter['NAMEGENQIDS'])) {
                    ...
                    if (stristr($alter['NAMEGENQIDS'], ",")) {
                        ...
                        $newQids = array();
                        foreach ($qIds as $qId) {
                            if (isset($questionTitles[$qId]) && isset($newData['newQuestionIds'][$questionTitles[$qId]])) {
                        ...
                        if (isset($questionTitles[$alter['NAMEGENQIDS']]) && isset($newData['newQuestionIds'][$questionTitles[$alter['NAMEGENQIDS']]])) {
        ...
        foreach ($data['answers'] as $answer) {
            $newAnswer = new Answer;
            if ($newData) {
                if (!isset($questionTitles[$answer['QUESTIONID']]) || !isset($newData['newQuestionIds'][$questionTitles[$answer['QUESTIONID']]])) {
                    continue;
                }
