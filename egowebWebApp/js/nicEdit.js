...
var nicEditorInstance = bkClass.extend({
	...
	construct : function(e,options,nicEditor) {
		...
		if(isTextarea || this.options.hasPanel) {
			...
			if(isTextarea) {
				var varMatch = e.value.match(/<VAR[^\/]+[^>]+\/>/gm);
				if(varMatch){
					varSnips = [];
					for(k in varMatch)
						varSnips.push(varMatch[k]);
					for(i = 0; i < varSnips.length;i++){
						e.value = e.value.replace(varSnips[i],"<img id='var_" +i+"' src='/images/var.png'>");
					}
				}
				var calcMatch = e.value.match(/<CALC[^\/]+[^>]+\/>/gm);
				if(calcMatch){
					calcSnips = [];
					for(k in calcMatch)
						calcSnips.push(calcMatch[k]);
					for(i = 0; i < calcSnips.length;i++){
						e.value = e.value.replace(calcSnips[i],"<img id='calc_" +i+"' src='/images/calc.png'>");
					}
				}
				var countMatch = e.value.match(/<COUNT[^\/]+[^>]+\/>/gm);
				if(countMatch){
					countSnips = [];
					for(k in countMatch)
						countSnips.push(countMatch[k]);
					for(i = 0; i < countSnips.length;i++){
						e.value = e.value.replace(countSnips[i],"<img id='count_" +i+"' src='/images/count.png'>");
					}
				}
				var containsMatch = e.value.match(/<CONTAINS[^\/]+[^>]+\/>/gm);
				if(containsMatch){
					containsSnips = [];
					for(k in containsMatch)
						containsSnips.push(containsMatch[k]);
					for(i = 0; i < containsSnips.length;i++){
						e.value = e.value.replace(containsSnips[i],"<img id='contains_" +i+"' src='/images/contains.png'>");
					}
				}
				console.log(e.value);
				var ifMatch = e.value.match(/<IF((.|\n)*)\/>/gm);
				if(ifMatch){
					ifSnips = [];
					for(k in ifMatch)
						ifSnips.push(ifMatch[k]);
					for(i = 0; i < ifSnips.length;i++){
						e.value = e.value.replace(ifSnips[i],"<img id='if_" +i+"' src='/images/if.png'>");
					}
				}
	...
	saveContent : function() {
		if(this.copyElm || this.options.hasPanel) {
			...
			if(typeof ifSnips != "undefined"){
				for(i = 0; i < ifSnips.length;i++){
					saved = saved.replace("<img id=\"if_" +i+"\" src=\"/images/if.png\">", ifSnips[i]);
				}
			}
			if(typeof calcSnips != "undefined"){
				for(i = 0; i < calcSnips.length;i++){
					if(saved.match("<img id=\"calc_" +i+"\" src=\"/images/calc.png\">"))
						saved = saved.replace("<img id=\"calc_" +i+"\" src=\"/images/calc.png\">", calcSnips[i]);
					if(saved.match("<img id='calc_" +i+"' src='/images/calc.png'>"))
						saved = saved.replace("<img id='calc_" +i+"' src='/images/calc.png'>", calcSnips[i]);
				}
			}
			...
			if(typeof varSnips != "undefined"){
				for(i = 0; i < varSnips.length;i++){
					if(saved.match("<img id=\"var_" +i+"\" src=\"/images/var.png\">"))
						saved = saved.replace("<img id=\"var_" +i+"\" src=\"/images/var.png\">", varSnips[i]);
					if(saved.match("<img id='var_" +i+"' src='/images/var.png'>"))
						saved = saved.replace("<img id='var_" +i+"' src='/images/var.png'>", varSnips[i]);
				}
			}
			...
			if(typeof countSnips != "undefined"){
				for(i = 0; i < countSnips.length;i++){
					if(saved.match("<img id=\"count_" +i+"\" src=\"/images/count.png\">"))
						saved = saved.replace("<img id=\"count_" +i+"\" src=\"/images/count.png\">", countSnips[i]);
					if(saved.match("<img id='count_" +i+"' src='/images/count.png'>"))
						saved = saved.replace("<img id='count_" +i+"' src='/images/count.png'>", countSnips[i]);
				}
			}
			...
			if(typeof containsSnips != "undefined"){
				for(i = 0; i < containsSnips.length;i++){
					if(saved.match("<img id=\"contains_" +i+"\" src=\"/images/contains.png\">"))
						saved = saved.replace("<img id=\"contains_" +i+"\" src=\"/images/contains.png\">", countSnips[i]);
					if(saved.match("<img id='contains_" +i+"' src='/images/contains.png'>"))
						saved = saved.replace("<img id='contains_" +i+"' src='/images/contains.png'>", countSnips[i]);
				}
			}
			this.ne.fireEvent('save',this);
			(this.copyElm) ? this.copyElm.value = saved : this.e.innerHTML = saved;
		}	
	},
...
var nicCodeButton = nicEditorAdvancedButton.extend({
	...
	addPane : function() {
		withCode = this.ne.selectedInstance.getContent();
		...
		this.addForm({
			...
			'code' : {type : 'content', 'value' : withCode, style : {width: '340px', height : '200px'}}
		});
	},
	...
	submit : function(e) {
		...
		console.log(code);
				var varMatch =code.match(/<VAR[^\/]+[^>]+\/>/gm);
				if(varMatch){
					varSnips = [];
					for(k in varMatch)
						varSnips.push(varMatch[k]);
					for(i = 0; i < varSnips.length;i++){
						code = code.replace(varSnips[i],"<img id='var_" +i+"' src='/images/var.png'>");
					}
				}
				var calcMatch = code.match(/<CALC[^\/]+[^>]+\/>/gm);
				if(calcMatch){
					calcSnips = [];
					for(k in calcMatch)
						calcSnips.push(calcMatch[k]);
					for(i = 0; i < calcSnips.length;i++){
						code = code.replace(calcSnips[i],"<img id='calc_" +i+"' src='/images/calc.png'>");
					}
				}
				var countMatch = code.match(/<COUNT[^\/]+[^>]+\/>/gm);
				if(countMatch){
					countSnips = [];
					for(k in countMatch)
						countSnips.push(countMatch[k]);
					for(i = 0; i < countSnips.length;i++){
						code = code.replace(countSnips[i],"<img id='count_" +i+"' src='/images/count.png'>");
					}
				}
				var containsMatch = code.match(/<CONTAINS[^\/]+[^>]+\/>/gm);
				if(containsMatch){
					containsSnips = [];
					for(k in containsMatch)
						containsSnips.push(containsMatch[k]);
					for(i = 0; i < containsSnips.length;i++){
						code = code.replace(containsSnips[i],"<img id='contains_" +i+"' src='/images/contains.png'>");
					}
				}
				var ifMatch = code.match(/<IF((.|\n)*)\/>/gm);
				if(ifMatch){
					ifSnips = [];
					for(k in ifMatch)
						ifSnips.push(ifMatch[k]);
					for(i = 0; i < ifSnips.length;i++){
						code = code.replace(ifSnips[i],"<img id='if_" +i+"' src='/images/if.png'>");
					}
				}
